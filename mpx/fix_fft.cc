...
extern fixed const bmp_spectrum_Sinewave[N_WAVE];       /* placed at end of this file for clarity */
extern fixed const bmp_spectrum_Loudampl[N_LOUD];

static int bmp_spectrum_db_from_ampl (fixed re, fixed im);
static fixed bmp_spectrum_fix_mpy (fixed a, fixed b);
 ...
 */
int
bmp_spectrum_fix_fft (fixed fr[], fixed fi[], int m, int inverse)
{
  ...
  while (l < n) {
    ...
    for (m = 0; m < l; ++m) {
      j = m << k;
      /* 0 <= j < N_WAVE/2 */
      wr = bmp_spectrum_Sinewave[j + N_WAVE / 4];
      wi = -bmp_spectrum_Sinewave[j];
      if (inverse)
        wi = -wi;
      if (shift) {
        wr >>= 1;
      ...
      for (i = m; i < n; i += istep) {
        j = i + l;
        tr = bmp_spectrum_fix_mpy (wr, fr[j]) -
            bmp_spectrum_fix_mpy (wi, fi[j]);
        ti = bmp_spectrum_fix_mpy (wr, fi[j]) +
            bmp_spectrum_fix_mpy (wi, fr[j]);
        qr = fr[i];
        qi = fi[i];
        if (shift) {
          qr >>= 1;
...
void
bmp_spectrum_window (fixed fr[], int n)
{
  int i, j, k;
  ...
  for (i = 0, k = N_WAVE / 4; i < n; ++i, k += j)
    FIX_MPY (fr[i], fr[i], 16384 - (bmp_spectrum_Sinewave[k] >> 1));
  n <<= 1;
  for (k -= j; i < n; ++i, k -= j)
    FIX_MPY (fr[i], fr[i], 16384 - (bmp_spectrum_Sinewave[k] >> 1));
}
 ...
 */
void
bmp_spectrum_fix_loud (fixed loud[], fixed fr[], fixed fi[], int n,
    int scale_shift)
{
  int i, max;
  ...
  for (i = 0; i < n; ++i) {
    loud[i] = bmp_spectrum_db_from_ampl (fr[i], fi[i]) + scale_shift;
    if (loud[i] > max)
      loud[i] = max;
  }
}
 ...
 */
static int
bmp_spectrum_db_from_ampl (fixed re, fixed im)
{
  ...
  if (loud2[0] == 0) {
    loud2[0] =
        (long) bmp_spectrum_Loudampl[0] * (long) bmp_spectrum_Loudampl[0];
    for (i = 1; i < N_LOUD; ++i) {
      v = (long) bmp_spectrum_Loudampl[i] * (long) bmp_spectrum_Loudampl[i];
      loud2[i] = v;
 ...
 */
static fixed
bmp_spectrum_fix_mpy (fixed a, fixed b)
{
  FIX_MPY (a, a, b);
  return a;
}
 ...
 */
static int
bmp_spectrum_iscale (int value, int numer, int denom)
{
 ...
 */
static fixed
bmp_spectrum_fix_dot (fixed * hpa, fixed * pb, int n)
{
  fixed *pa = hpa;              /* FIXME */
  long sum;
  register fixed a, b;
...
ERROR:N_WAVE != 1024
#endif
fixed const bmp_spectrum_Sinewave[1024] = {
...
ERROR:N_LOUD != 100
#endif
fixed const bmp_spectrum_Loudampl[100] = {
