...
namespace MPX
{
	...
	void
	SourceController::source_install (ItemKey const& source_id)
	{
#if 0
		...
		m_Sources[source_id]->signal_caps().connect
		  (sigc::bind (sigc::mem_fun (*this, &SourceController::on_source_caps), source_id));

		m_Sources[source_id]->signal_flags().connect
		  (sigc::bind (sigc::mem_fun (*this, &SourceController::on_source_flags), source_id));

		m_Sources[source_id]->signal_playback_request().connect
		  (sigc::bind (sigc::mem_fun (*this, &SourceController::on_source_play_request), source_id));

		m_Sources[source_id]->signal_stop_request().connect
		  (sigc::bind (sigc::mem_fun (*this, &SourceController::on_source_stop), source_id));

		m_Sources[source_id]->signal_play_async().connect
		  (sigc::bind (sigc::mem_fun (*this, &SourceController::play_async_cb), source_id));

		m_Sources[source_id]->signal_next_async().connect
		  (sigc::bind (sigc::mem_fun (*this, &SourceController::next_async_cb), source_id));

		m_Sources[source_id]->signal_prev_async().connect
		  (sigc::bind (sigc::mem_fun (*this, &SourceController::prev_async_cb), source_id));

		m_Sources[source_id]->signal_segment().connect
		  (sigc::bind (sigc::mem_fun (*this, &SourceController::on_source_segment), source_id));

		m_Sources[source_id]->signal_track_metadata().connect
		  (sigc::bind (sigc::mem_fun (*this, &SourceController::on_source_track_metadata), source_id));

		m_Sources[source_id]->signal_items().connect
		  (sigc::bind (sigc::mem_fun (*this, &SourceController::on_source_items), source_id));
    ...
    void
    SourceController::on_source_items(gint64 count, ItemKey const& key)
    {
#if 0
    ...
    PyObject*
    SourceController::get_source(std::string const& uuid)
    {
#if 0
    ...
    PyObject*
    SourceController::get_sources_by_class(std::string const& uuid)
    {
#if 0
        using namespace boost::python;
        boost::python::list l;
    ...
    void
    SourceController::add_subsource(PlaybackSource* p, ItemKey const& parent, gint64 id)
    {
#if 0
        Gtk::Alignment * a = new Gtk::Alignment;
	...
	void
	SourceController::on_source_caps (Caps caps, ItemKey const& source_id)
	{
#if 0
	...
	void
	SourceController::on_source_flags (Flags flags, ItemKey const& source_id)
	{
#if 0
	  Mutex::Lock L (m_SourceCFLock);
	...
	void
	SourceController::on_source_play_request (ItemKey const& source_id)
	{
#if 0
	  if( m_ActiveSource ) 
	  {
	...
	void
	SourceController::on_source_segment (ItemKey const& source_id)
	{
#if 0
	...
	void
	SourceController::on_source_stop (ItemKey const& source_id)
	{
#if 0
	  g_return_if_fail( m_ActiveSource == source_id );
      stop ();
	...
	void
	SourceController::on_source_changed (ItemKey const& source_id)
	{
#if 0
	    Mutex::Lock L (m_SourceCFLock);
	...
	void
	SourceController::play_async_cb (ItemKey const& source_id)
	{
#if 0
	  PlaybackSource* source = m_Sources[source_id];
	...
	void
	SourceController::next_async_cb (ItemKey const& source_id)
	{
#if 0
	  PlaybackSource* source = m_Sources[source_id];
	...
	void
	SourceController::prev_async_cb (ItemKey const& source_id)
	{
#if 0
	  PlaybackSource* source = m_Sources[source_id];
	  Flags f = m_source_f[source_id];
