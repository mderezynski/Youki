 ...
 * If <link linkend="MPXSpectrum--message">message property</link> is #TRUE it
 * sends analysis results as application message named
 * <classname>&quot;spectrum&quot;</classname> after each interval of time given
 * by the <link linkend="MPXSpectrum--interval">interval property</link>.
 ...
 *   <link linkend="MPXSpectrum--threshold">threshold property.</link>.
 *   </para>
 * </listitem>
 *
...
GST_DEBUG_CATEGORY_STATIC (bmp_spectrum_debug);
#define GST_CAT_DEFAULT bmp_spectrum_debug

/* elementfactory information */
static const GstElementDetails bmp_spectrum_details =
...
GST_BOILERPLATE (MPXSpectrum, bmp_spectrum, GstBaseTransform,
    GST_TYPE_BASE_TRANSFORM);

static void bmp_spectrum_dispose (GObject * object);
static void bmp_spectrum_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void bmp_spectrum_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);
static gboolean bmp_spectrum_set_caps (GstBaseTransform * trans, GstCaps * in,
    GstCaps * out);
static gboolean bmp_spectrum_start (GstBaseTransform * trans);
static gboolean bmp_spectrum_stop (GstBaseTransform * trans);
static gboolean bmp_spectrum_event (GstBaseTransform * trans, GstEvent * event);
static GstFlowReturn bmp_spectrum_transform_ip (GstBaseTransform * trans,
    GstBuffer * in);


#define fixed short
extern int bmp_spectrum_fix_fft (fixed fr[], fixed fi[], int m, int inverse);
extern void bmp_spectrum_fix_loud (fixed loud[], fixed fr[], fixed fi[], int n,
    int scale_shift);
extern void bmp_spectrum_window (fixed fr[], int n);

static void
bmp_spectrum_base_init (gpointer g_class)
{
  ...
  gst_element_class_set_details (element_class, &bmp_spectrum_details);
}

static void
bmp_spectrum_class_init (MPXSpectrumClass * klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  GstBaseTransformClass *trans_class = GST_BASE_TRANSFORM_CLASS (klass);

  gobject_class->set_property = bmp_spectrum_set_property;
  gobject_class->get_property = bmp_spectrum_get_property;
  gobject_class->dispose = bmp_spectrum_dispose;

  trans_class->set_caps = GST_DEBUG_FUNCPTR (bmp_spectrum_set_caps);
  trans_class->start = GST_DEBUG_FUNCPTR (bmp_spectrum_start);
  trans_class->stop = GST_DEBUG_FUNCPTR (bmp_spectrum_stop);
  trans_class->event = GST_DEBUG_FUNCPTR (bmp_spectrum_event);
  trans_class->transform_ip = GST_DEBUG_FUNCPTR (bmp_spectrum_transform_ip);
  ...
  GST_DEBUG_CATEGORY_INIT (bmp_spectrum_debug, "bmpx-spectrum", 0,
      "audio spectrum analyser element");
}

static void
bmp_spectrum_init (MPXSpectrum * spectrum, MPXSpectrumClass * g_class)
{
  ...
  spectrum->loud = (gint16*)g_malloc (SPECTRUM_WINDOW_LEN * sizeof (gint16));
  spectrum->im = (gint16*)g_malloc0 (SPECTRUM_WINDOW_LEN * sizeof (gint16));
  spectrum->re = (gint16*)g_malloc0 (SPECTRUM_WINDOW_LEN * sizeof (gint16));
  spectrum->spect = (guchar*)g_malloc (spectrum->bands * sizeof (guchar));
}

static void
bmp_spectrum_dispose (GObject * object)
{
  MPXSpectrum *spectrum = MPX_SPECTRUM (object);
...
static void
bmp_spectrum_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  MPXSpectrum *filter = MPX_SPECTRUM (object);
  ...
  switch (prop_id) {
    ...
    case PROP_BANDS:
      ...
      g_free (filter->spect);
      filter->spect = (guchar*)g_malloc (filter->bands * sizeof (guchar));
...
static void
bmp_spectrum_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  MPXSpectrum *filter = MPX_SPECTRUM (object);
...
static gboolean
bmp_spectrum_set_caps (GstBaseTransform * trans, GstCaps * in, GstCaps * out)
{
  MPXSpectrum *filter = MPX_SPECTRUM (trans);
...
static gboolean
bmp_spectrum_start (GstBaseTransform * trans)
{
  MPXSpectrum *filter = MPX_SPECTRUM (trans);
...
static gboolean
bmp_spectrum_stop (GstBaseTransform * trans)
{
  MPXSpectrum *filter = MPX_SPECTRUM (trans);
...
static gboolean
bmp_spectrum_event (GstBaseTransform * trans, GstEvent * event)
{
  MPXSpectrum *filter = MPX_SPECTRUM (trans);
...
static GstMessage *
bmp_spectrum_message_new (MPXSpectrum * spectrum, GstClockTime endtime)
{
  GstStructure *s;
  GValue v = { 0, };
  GValue *l;
...
bmp_spectrum_transform_ip (GstBaseTransform * trans, GstBuffer * in)
{
  MPXSpectrum *spectrum = MPX_SPECTRUM (trans);
  guint wanted;
  gint i, j, k;
  gint32 acc;
  gfloat pos, step;
  ...
  while (gst_adapter_available (spectrum->adapter) >= wanted) {
    ...
    bmp_spectrum_window (spectrum->re, SPECTRUM_WINDOW_LEN);
    bmp_spectrum_fix_fft (spectrum->re, spectrum->im, SPECTRUM_WINDOW_BASE,
        FALSE);
    bmp_spectrum_fix_loud (spectrum->loud, spectrum->re, spectrum->im,
    ...
    if (spectrum->num_frames >=
      ...
      if (spectrum->message) {
        GstMessage *m = bmp_spectrum_message_new (spectrum, endtime);
...
{
  return gst_element_register (plugin, "bmpx-spectrum", GST_RANK_NONE,
      MPX_TYPE_SPECTRUM);
}
...
GST_PLUGIN_DEFINE_STATIC (GST_VERSION_MAJOR,
    ...
    "bmpx-spectrum",
    "Run an FFT on the audio signal, output spectrum data",
    plugin_init, VERSION, "LGPL", "MPXx", "http://beep-media-player.org")
