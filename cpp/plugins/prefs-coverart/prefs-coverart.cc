        ...
        public:
                ...
                : WidgetLoader<Gtk::TreeView>(xml, "preferences-treeview-coverartsources")
            {
                ...
                for( unsigned i = 0; i < G_N_ELEMENTS(sources); ++i )
                {
                    ...
                    int source = mcs->key_get<int>("PrefsCoverart-CoverArtSources", (boost::format ("Source%d") % i).str());

                    (*iter)[Columns.Name]   = _(sources[source]);
                    (*iter)[Columns.ID]     = source;
                    (*iter)[Columns.Active] = mcs->key_get<bool>("PrefsCoverart-CoverArtSources", (boost::format ("SourceActive%d") % i).str());
                }
                ...
                update_configuration ()
            {
                ...
                for(TreeNodeChildren::const_iterator i = children.begin(); i != children.end(); ++i)
                {
                    ...
                    mcs->key_set<int>("PrefsCoverart-CoverArtSources", (boost::format ("Source%d") % n).str(), (*i)[Columns.ID]);
                    mcs->key_set<bool>("PrefsCoverart-CoverArtSources", (boost::format ("SourceActive%d") % n).str(), (*i)[Columns.Active]);
                }
            }
