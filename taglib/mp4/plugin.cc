...
extern "C" bool _set (std::string const& filename, Track & track)
{
  ...
  if (track[ATTRIBUTE_ALBUM_ARTIST])
    ItemMap["aART"] =  StringList (String (get<std::string>(track[ATTRIBUTE_ALBUM_ARTIST].get()), String::UTF8));
                       
  if (track[ATTRIBUTE_MB_TRACK_ID])
    ItemMap["----:com.apple.iTunes:MusicBrainz Track Id"] = 
                       StringList (String (get<std::string>(track[ATTRIBUTE_MB_TRACK_ID].get()), String::UTF8));

  if (track[ATTRIBUTE_MB_ALBUM_ID])
    ItemMap["----:com.apple.iTunes:MusicBrainz Album Id"] =
                       StringList (String(get<std::string>(track[ATTRIBUTE_MB_ALBUM_ID].get()), String::UTF8));

  if (track[ATTRIBUTE_MB_ARTIST_ID])
    ItemMap["----:com.apple.iTunes:MusicBrainz Artist Id"] = 
                       StringList (String (get<std::string>(track[ATTRIBUTE_MB_ARTIST_ID].get()), String::UTF8));

  if (track[ATTRIBUTE_MB_ALBUM_ARTIST_ID])
    ItemMap["----:com.apple.iTunes:MusicBrainz Album Artist Id"] =
                       StringList (String (get<std::string>(track[ATTRIBUTE_MB_ALBUM_ARTIST_ID].get()), String::UTF8));

  if (track[ATTRIBUTE_ALBUM_ARTIST_SORTNAME])
    ItemMap["soaa"] =  StringList (String (get<std::string>(track[ATTRIBUTE_ALBUM_ARTIST_SORTNAME].get()), String::UTF8));
                       
  if (track[ATTRIBUTE_ARTIST_SORTNAME])
    ItemMap["soar"] =  StringList (String (get<std::string>(track[ATTRIBUTE_ARTIST_SORTNAME].get()), String::UTF8));

  if (track[ATTRIBUTE_MB_RELEASE_DATE])
    ItemMap["\251day"] =  
                       StringList (String (get<std::string>(track[ATTRIBUTE_MB_RELEASE_DATE].get()), String::UTF8));

  if (track[ATTRIBUTE_ASIN])
    ItemMap["----:com.apple.iTunes:ASIN"] = 
                       StringList (String (get<std::string>(track[ATTRIBUTE_ASIN].get()), String::UTF8));

  if (track[ATTRIBUTE_MUSICIP_PUID])
    ItemMap["----:com.apple.ITunes:MusicIP PUID"] = 
                       StringList (String (get<std::string>(track[ATTRIBUTE_MUSICIP_PUID].get()), String::UTF8));
