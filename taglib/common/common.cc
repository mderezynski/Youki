...
namespace MPX
{
    ...
    metadata_get_common (TagLib::File * file, Track & track)
    {
      ...
      {
          track [ ATTRIBUTE_BITRATE ] = gint64 (file->audioProperties()->bitrate());
          track [ ATTRIBUTE_SAMPLERATE ] = gint64 (file->audioProperties()->sampleRate());
          track [ ATTRIBUTE_TIME ] = gint64 (file->audioProperties()->length());
      }
      else
      {
      ...
      track [ ATTRIBUTE_TRACK ] = gint64 (file->tag()->track());
      track [ ATTRIBUTE_DATE ] = gint64 (file->tag()->year());
    ...
    metadata_set_common (TagLib::File * file, Track & track)
    {
      ...
      if (tag)
      {
        ...
        if (track[ATTRIBUTE_DATE])
            tag->setYear    (boost::get<gint64>(track[ATTRIBUTE_DATE].get()));

        if (track[ATTRIBUTE_TRACK])
            tag->setTrack   (boost::get<gint64>(track[ATTRIBUTE_TRACK].get()));
      }
      else
      {
        try{
