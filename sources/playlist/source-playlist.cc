    ...
    struct PlaylistPrivate
    {
            ...
            :   public Gtk::TreeView
        {
            ...
            public:
              ...
              void
              drag_data (const Glib::RefPtr<Gdk::DragContext>& context,
        ...
        PlaylistPrivate (MPX::Player & player, MPX::Source::PlaybackSourcePlaylist & mlib)
        {
            ...
            m_Playlist = new PlaylistTreeView(m_Lib, m_HAL, mlib);
...
namespace Source
{
    ...
    : PlaybackSource(ui_manager, name, C_CAN_SEEK)
    {
        ...
        m_Private->m_Playlist->signal_row_activated().connect( sigc::mem_fun( *this, &PlaybackSourcePlaylist::on_plist_row_activated ) );
        m_Private->m_Playlist->signal_query_tooltip().connect( sigc::mem_fun( *this, &PlaybackSourcePlaylist::on_plist_query_tooltip ) );
    }
    ...
    {
        m_Private->m_Playlist->m_CurrentIter = m_Private->m_Playlist->ListStore->get_iter( path );
        m_Private->m_Playlist->m_CurrentId = (*m_Private->m_Playlist->m_CurrentIter.get())[m_Private->m_Playlist->PlaylistColumns.RowId];
        m_Private->m_Playlist->queue_draw();
    ...
    PlaybackSourcePlaylist::on_plist_query_tooltip(int x,int y, bool kbd, const Glib::RefPtr<Gtk::Tooltip> & tooltip)
    {
        ...
        TreeViewColumn *col;
        int x2, y2;
        m_Private->m_Playlist->convert_widget_to_bin_window_coords(x, y, x2, y2);
        if(m_Private->m_Playlist->get_path_at_pos (x2, y2, path, col, cell_x, cell_y))
        {
            TreeIter iter = m_Private->m_Playlist->ListStore->get_iter(path);
            GtkTreeIter const* iter_c = iter->gobj();
            g_signal_emit(G_OBJECT(gobj()), signals[PSM_SIGNAL_PLAYLIST_TOOLTIP], 0, G_OBJECT(m_Private->m_Playlist->ListStore->gobj()), iter_c, &tooltip_widget);
            if( tooltip_widget )
            {
    ...
    {
        GtkListStore * store = GTK_LIST_STORE(m_Private->m_Playlist->ListStore->gobj());
    ...
    PlaybackSourcePlaylist::get_playlist_model ()
    {
        return m_Private->m_Playlist->ListStore;
    }

    PyObject*
    PlaybackSourcePlaylist::get_playlist_current_iter ()
    {
        if(!m_Private->m_Playlist->m_CurrentIter)
        {
            Py_INCREF(Py_None);
            return Py_None;
        }

        GtkTreeIter  * iter = m_Private->m_Playlist->m_CurrentIter.get().gobj();
    ...
    {
        m_Private->m_Playlist->clear();
        m_Private->m_Playlist->append_album(id);
        Signals.PlayRequest.emit();
    }
    ...
    {
        m_Private->m_Playlist->clear();
        m_Private->m_Playlist->append_tracks(idv, NO_ORDER);
        Signals.PlayRequest.emit();
    }
    ...
    {
        g_message(G_STRFUNC);
        m_Private->m_Playlist->m_CurrentIter = boost::optional<Gtk::TreeIter>(); 
        m_Private->m_Playlist->m_CurrentId = boost::optional<gint64>();
        Signals.PlayRequest.emit();
    }
    ...
    {
        g_return_val_if_fail(m_Private->m_Playlist->m_CurrentIter, std::string());
        
#ifdef HAVE_HAL
        try{
            MPX::Track t ((*m_Private->m_Playlist->m_CurrentIter.get())[m_Private->m_Playlist->PlaylistColumns.MPXTrack]);
        ...
        return std::string();
#else
        return Glib::ustring((*m_Private->m_Playlist->m_CurrentIter.get())[m_Private->m_Playlist->PlaylistColumns.Location]);
#endif // HAVE_HAL
    }
    ...
    {
        std::vector<Gtk::TreePath> rows = m_Private->m_Playlist->get_selection()->get_selected_rows();

        if(m_Private->m_Playlist->m_PlayInitIter)
        {
            m_Private->m_Playlist->m_CurrentIter = m_Private->m_Playlist->m_PlayInitIter.get();
            m_Private->m_Playlist->m_PlayInitIter.reset ();
        }
        // NO else so we fall through to the last check
        if(!rows.empty() && !m_Private->m_Playlist->m_CurrentIter)
        {
            m_Private->m_Playlist->m_CurrentIter = m_Private->m_Playlist->ListStore->get_iter (rows[0]); 
            m_Private->m_Playlist->m_CurrentId = (*m_Private->m_Playlist->m_CurrentIter.get())[m_Private->m_Playlist->PlaylistColumns.RowId];
            m_Private->m_Playlist->queue_draw();
            return true;
        }
        ...
        if(m_Private->m_Playlist->m_CurrentIter)
        {
            TreeIter &iter = m_Private->m_Playlist->m_CurrentIter.get();
            TreePath path1 = m_Private->m_Playlist->ListStore->get_path(iter);
            TreePath path2 = m_Private->m_Playlist->ListStore->get_path(--m_Private->m_Playlist->ListStore->children().end());
    ...
    {
        g_return_val_if_fail(bool(m_Private->m_Playlist->m_CurrentIter), false);

        TreeIter & iter = m_Private->m_Playlist->m_CurrentIter.get();

        TreePath path1 = m_Private->m_Playlist->ListStore->get_path(iter);
        TreePath path2 = m_Private->m_Playlist->ListStore->get_path(m_Private->m_Playlist->ListStore->children().begin());
    ...
    PlaybackSourcePlaylist::check_caps ()
    {
        if (m_Private->m_Playlist->m_CurrentIter)
        {
            ...
            TreePath path1 = m_Private->m_Playlist->ListStore->get_path(iter);
            TreePath path2 = m_Private->m_Playlist->ListStore->get_path(m_Private->m_Playlist->ListStore->children().begin());
            TreePath path3 = m_Private->m_Playlist->ListStore->get_path(--m_Private->m_Playlist->ListStore->children().end());
        ...
        gint count = gtk_tree_model_iter_n_children(GTK_TREE_MODEL(m_Private->m_Playlist->ListStore->gobj()), NULL);
    ...
    {
        g_return_if_fail(bool(m_Private->m_Playlist->m_CurrentIter));

        MPX::Metadata m = (MPX::Track((*m_Private->m_Playlist->m_CurrentIter.get())[m_Private->m_Playlist->PlaylistColumns.MPXTrack]));
    ...
    PlaybackSourcePlaylist::play_post () 
    {
        ...
        m_Private->m_Playlist->scroll_to_track();
    }
    ...
    {
        m_Private->m_Playlist->prepare_uris (uris, play);
        if(play)
        {
            Signals.PlayRequest.emit();
        }
    ...
    void
    PlaybackSourcePlaylist::drag_data (const Glib::RefPtr<Gdk::DragContext>& context,
                                       const Gtk::SelectionData& data, guint, guint)
    {
        m_Private->m_Playlist->drag_data(context, data);
    }
